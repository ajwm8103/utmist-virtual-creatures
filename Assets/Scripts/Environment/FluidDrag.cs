...
public class FluidDrag : MonoBehaviour
{
	...
	Vector3 fluidDragVecPosZ = -forward * dragScaling * 0.5 * sa_z * 1.05 * 1000 * Mathf.Pow(Vector3.Dot(forward, pointVelPosZ), 2);
	...
	Vector3 fluidDragVecPosY = -up * dragScaling * 0.5 * sa_y * 1.05 * 1000 * Mathf.Pow(Vector3.Dot(up, pointVelPosY), 2);
	...
	Vector3 fluidDragVecPosX = -right * dragScaling * 0.5 * sa_x * 1.05 * 1000 * Mathf.Pow(Vector3.Dot(right, pointVelPosX), 2);
