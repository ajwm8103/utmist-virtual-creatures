...
public class FluidDrag : MonoBehaviour
{
	...
	Vector3 fluidDragVecPosZ = -forward * scale * 0.5 * sa_z * 1.05 * 1000 * Math.Pow(Vector3.Dot(forward, pointVelPosZ), 2);
	...
	Vector3 fluidDragVecPosY = -up * scale * 0.5 * sa_y * 1.05 * 1000 * Math.Pow(Vector3.Dot(up, pointVelPosY), 2);
	...
	Vector3 fluidDragVecPosX = -right * scale * 0.5 * sa_x * 1.05 * 1000 * Math.Pow(Vector3.Dot(right, pointVelPosX), 2);
